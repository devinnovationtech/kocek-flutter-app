...
import 'package:kocek_app/env/widget/page_view/size_reporting_widget.dart';

typedef WidgetBuilder = Widget Function(BuildContext context, int index);

class KocekPageView extends StatefulWidget {
  ...
  ///
  /// By default (false) [KocekPageView] will resize to the size of it's
  ...
  /// [KocekPageView] size animation. This can lead to a behaviour
  /// when after changing the page, [KocekPageView] will first shrink to 0.0
  /// and then animate to the size of the page.
  ///
  /// For example: If there is certainty that most pages displayed by [KocekPageView]
  ...
  @override
  _KocekPageViewState createState() => _KocekPageViewState();
}

class _KocekPageViewState extends State<KocekPageView> {
  ...
  @override
  void didUpdateWidget(covariant KocekPageView oldWidget) {
  ...
  bool _shouldReinitializeHeights(KocekPageView oldWidget) {
